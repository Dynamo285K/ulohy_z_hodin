...
class Line:
    def __init__(self, point: tuple, dx: int, dy: int, colour: str, canvas):
        ...
        self.id_3 = None
        if dx < dy:
            self.id_1 = canvas.create_polygon(point[0], point[1], point[0] + dx // 2, point[1] - dx//2, point[0] + dx,
        ...
        if dx == dy:
            self.id_3 = canvas.create_rectangle(point[0], point[1], point[0] + dx, point[1] + dy, fill=colour, outline="white")
        ...
        for i in range(6):
            if i > 3:
                ...
                if i == 0 or i == 2 or i == 4:
                    ...
                    x = i * (140 + spacing_factor * 140)
                y = 100
                segment = Segment((x, y), 20, 100, 20, 100, "black", canvas)
                self.segments.append(segment)
                if i == 3 or i == 1:
                    col_1 = Line((x + 160,y + 140), 20, 20, "black", canvas)
                    col_2 = Line((x + 160,y + 100), 20, 20, "black", canvas)
                    col_1.on()
                    col_2.on()
